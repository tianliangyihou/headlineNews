...
- (void)showWithURL:(NSURL *)url andwithAnimation:(BOOL)animation andWithStatusModel:(id)model{
    ...
    BOOL needDownLoad = [[self.statusModel valueForKey:@"needDownLoad"] boolValue];

    if (!needDownLoad) {
        if (_loadingView) {
            [_loadingView removeFromSuperview];
        }
        [self adjustImageViewStatusWithShowImage:currentImage];// 第一动画图片存在
    }else if(needDownLoad && currentImage) {

             if (_loadingView) {
                 [_loadingView removeFromSuperview];
             }
             [self adjustdownLoadedImage:currentImage];
    }else {
        ...
        [self adjustImageViewStatusWithPlaceholdImage:currentImage];// 第一动画图片不存在
    }
}

- (void)adjustImageViewStatusWithShowImage:(UIImage *)image {
    weak_self;
    ...
    [self resetImage:image andWithContentSize:photoImageViewFrame.size];
}

- (void)adjustImageViewStatusWithPlaceholdImage:(UIImage *)image {
    weak_self;
    [[SDWebImageManager sharedManager].imageDownloader setSuspended:YES];
    self.imageSize = [LBPhotoBrowserManager defaultManager].helper.currentShowView.frame.size;
    [self resetScrollViewStatus];
    CGRect photoImageViewFrame;
    photoImageViewFrame.origin = CGPointZero;
    photoImageViewFrame.size = self.imageSize;
    if (self.shouldAnimation && [LBPhotoBrowserManager defaultManager].showBrowserWithAnimation) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            [wself layoutSubviews];// sometime need layout
            [wself loadingView];
            [[SDWebImageManager sharedManager].imageDownloader setSuspended:NO];
        }];
...
- (void)adjustdownLoadedImage:(UIImage *)image {
    weak_self;
    self.imageSize = [self newSizeForImageViewWithImage:image];
    [self resetScrollViewStatus];
    CGRect photoImageViewFrame;
...
- (void)resetImage:(UIImage *)image andWithContentSize:(CGSize)size{
