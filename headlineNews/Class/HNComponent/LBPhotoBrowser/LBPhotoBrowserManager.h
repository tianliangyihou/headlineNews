...
/**
 下一版待更新
 1@property (nonatomic , strong)NSArray *imageViews;
 
 2 if (mgr.placeHoldImageCallBackBlock) {
 ...
 3 图片在加载中动画中 不显示loading动画
 dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.25 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
 self.hidden = NO;
 [self timer];
 });
 
 4 self.contentMode = UIViewContentModeScaleAspectFit;
